...
- SQLite による `track_map` / `config` テーブル作成
- 右クリックメニュー `Spotify Linker` からの Spotify track URI 手動登録、自動登録、削除
...
- album URL / URI から track URL / URI を自動取得して登録
...
- `Set Track URI from Album URL...`: Spotify album URL / URI から、選択トラックの `TRACKNUMBER` に対応する Spotify track URI を取得して登録
- `Auto Set Track URI from Album`: 選択トラックのアルバムタグから Spotify album を検索し、対応する Spotify track URI を取得して登録

album URL / URI は保存せず、登録時点で Spotify track URI に解決して `track_map` に保存します。たとえばローカルの 3 曲目に album URL を入れた場合、Spotify アルバムの 3 曲目の `spotify:track:...` が登録されます。
...
track は URI として追加します。album は Spotify Web API で track URI に展開して追加します。playlist は Spotify 側の現在再生と queue 先頭の次曲だけを `spotify:track:...` の仮想トラックとして追加します。
